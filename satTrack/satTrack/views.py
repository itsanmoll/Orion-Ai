...
from satTrack.extract_data import convert, get_live_data, data_over_time, get_position
...
def satellite_data_buffer(request, norad_id):
    try:
        ...
        return Response({'time_scale_positions': time_scale_pos})
    except Satellite.DoesNotExist:
        return Response({'error': 'Satellite not found'}, status=status.HTTP_404_NOT_FOUND)
...
@api_view(['POST']) # curl -X GET http://127.0.0.1:8000/api/satellites/
def satellite_detail(request):
    norad_id = request.data['norad_id']
    try:
        satellite = Satellite.objects.get(pk=norad_id)
